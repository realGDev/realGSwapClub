...
const change_GANGSTER_MasterChefMultiplier = async (newNum) => {
  ...
  const masterChef = new web3.eth.Contract(
    ...
    GMasterChefContract.networks[id].address
  );
...
const change_GANGSTER_POOL_0_ammoUsdc_alloc = async (newNum) => {
  let provider = new HDWalletProvider(
    [process.env.MNEMONIC],
    "https://rpc.energyweb.org"
  );
  ...
  try {
    const alloc = await masterChef.methods.set(0, newNum, true).send({
      from: addresses[0],
      gas: gas,
      gasPrice: gasPrice,
    });
    console.log(`ANSWER when AMMO-USDC allocPoint: ${alloc.toString()}`);
    console.log(`Staking alloc was set to ${newNum}`);
  } catch (e) {
    console.log(e.toString());
    console.log(`Was no able to change staking alloc to ${newNum}`);
  }
};

const change_GANGSTER_POOL_1_gsStaking_alloc = async (newNum) => {
  let provider = new HDWalletProvider(
    [process.env.MNEMONIC],
    "https://rpc.energyweb.org"
  );
  ...
  try {
    const alloc = await masterChef.methods.set(1, newNum, true).send({
      from: addresses[0],
      gas: gas,
      gasPrice: gasPrice,
    });
    console.log(`ANSWER when Staking allocPoint: ${alloc.toString()}`);
    console.log(`Staking alloc was set to ${newNum}`);
  } catch (e) {
    console.log(e.toString());
    console.log(`Was no able to change staking alloc to ${newNum}`);
...
const chooseWinnerLottoEwt = async () => {
  ...
  try {
    ...
    console.log(`This is the list of uniqueIDs: \n${array}`);
...
const checkWinner = async () => {
  ...
  const allLotoParticpants = [
    "0x9047552c3c149058ef559963d7fdfcf077e41fe3",
    "0xdf840147f7719d51f229f59d6dbd1b32744b364c",
    "0x6daf008eb61a575233fa319ee9ca21aa1ac7fdf6",
    "0xacdc1853a98a8e0d25afc4d02da1c85110d67bc0",
    "0xa5d041030efdd2c406ec94ce2cd7b1bb625f67a2",
    "0xafb8ccb4f9c7698f802fd10564388a58dc11eb19",
    "0x10dd48a9dda3e462c80c86e9c30c613324c321f0",
    "0x2d9b6dad16396fff43a4ecd058e0fc798fdf0181",
    "0x97c2be91d35772731eb0692d14870a173d3a676d",
    "0xc3c6606123af58c0ea928a84e9a5f279431a182d",
    "0x95a859a60dec9c0759ffaf794bc2f54bb9d1f46a",
    "0x04f9317158b1426ff65ec141c14cea73fb764b0e",
    "0xc40d842d8b57b7ede363f3c0f42c4d95bb5be34d",
    "0x635f953692bb98c1fae44edcadb4a67ef417f207",
    "0x1737512b4dc2ebe76d80972de09dffef67b89fe7",
    "0x867183b52d6c8014877bdaaed4598f09acebcea1",
    "0x0b767a0f68a3075e2335d4b61023b170d7ea8652",
    "0xf28a087e52e18791ab500239d919cf9cc0378acf",
    "0x875bf12ccc09d0a97885f8b328b5617d7e004e89",
    "0x38726205c717565c76a5f614c969e99c75455a45",
    "0x5b76ddde5a9b5d75425b325c4106d3ed086566c8",
    "0xb5d5776b40a50c0b2482d7e1f844511cb066e83c",
    "0x3918bb2104621c19f4b04a405836e19d6ae4f69e",
    "0x821e1f39c88f9cef70485906a4b1cdbfdcf57d61",
    "0xb172bcb3e36e7c44962f3c795c76d2cf15dc79b0",
    "0x322b46bd9847cd5f1189113fdb7916cb0f220489",
    "0x5f26304a4c9d8da51358355e8d3b8aae262c9449",
  ];
  ...
  console.log("Cheking winner through all participants");
  for (let index = 0; index < allLotoParticpants.length; index++) {
    try {
      const winner = await lotteryContract.methods
        .winnings(allLotoParticpants[index])
        .call();
      if (winner != 0) {
        console.log(
          `I'm glad to tell that the Address ${allLotoParticpants[
            index
...
// checkWinner();
//!PUG MasterChef
addPugPool("0xe99bdf79cb46416e28bbfd2dc9c8f8454d7bdc09", 2); //? (FUNCTION)  Add Pools (ADDRESS pool)
...
//TODO pid --> (4): '0xc376a565e670a31516bf81339f09153466b8f196','PUG-SLR', 1
//TODO pid --> (5): '0xe99bdf79cb46416e28bbfd2dc9c8f8454d7bdc09','PUG-AMMO', 2
// change_PUG_MasterChefMultiplier(1);

//!----end_PUG MasterChef

//!G$ MasterChef
// addGSPool("0xc6838a932e29283aa3b286306b3d3656748832f9", 2); //? (FUNCTION)  Add Pools (ADDRESS pool)
//TODO pid --> (0): '0x20ae3646e74dfec646b2788286065f642245ca5f','AMMO-USDC', 1
//TODO pid --> (1): '0x41c49ef86f513498D9Be19F4E920a6Afbe8Af4Cb','G$ Staking', 3
//TODO pid --> (2): '0xc6838a932e29283aa3b286306b3d3656748832f9','G$-USDC', 2
// change_GANGSTER_MasterChefMultiplier(2);
// change_GANGSTER_POOL_1_gsStaking_alloc(3);
// change_GANGSTER_POOL_0_ammoUsdc_alloc(1);
// getGSPoolData(1);
//!---end G$ MasterChef
